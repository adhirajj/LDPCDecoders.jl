...
# function get_check_neighbors(HT::SparseMatrixCSC, check_idx::Int)
#     neighbors = Int[] # collects connected variable nodes in this
#     # Use the transposed matrix to efficiently access rows as columns
#     for idx in nzrange(HT, check_idx) # where check_idx is the row it is examining in H
#         col = rowvals(HT)[idx]
#         if nonzeros(HT)[idx]
#             push!(neighbors, col) # For each col with a non-zero entry, it adds that col index to the neighbors list
#         end
#     end
#     return neighbors # List of all variables connected to this check node
# end
